...
function aventurien_solo_db_get_all_modules()
{
   	global $wpdb;
    $db_table_name_states = $wpdb->prefix . 'sonnenstrasse_solo_states';
	$db_table_name_heroes  = $wpdb->prefix . 'sonnenstrasse_heroes';
    
	$modules = $wpdb->get_results("SELECT DISTINCT module FROM $db_table_name_states ORDER BY module");
	foreach ($modules as $module)
	{
		$module->states = $wpdb->get_results("SELECT * FROM $db_table_name_states states INNER JOIN $db_table_name_heroes heroes ON states.hero_id = heroes.hero_id WHERE states.module='" . $module->module . "' ORDER BY heroes.name");
	}
